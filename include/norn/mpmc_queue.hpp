...
class mpmc_queue {
  ...
  static_assert(
      ...
      "mpmc_queue requires lock-free position and sequence atomics");
  static_assert(
      std::is_nothrow_destructible_v<T>,
      "mpmc_queue requires a noexcept destructor for T");
 ...
 public:
  ...
  bool emplace(Args&&... args) {
    static_assert(
        ...
        "mpmc_queue requires noexcept construction of T");
  ...
  bool try_pop(T& value) {
    static_assert(
        ...
        "mpmc_queue requires a nothrow move assignment for T");
  ...
  [[nodiscard]] std::optional<T> try_pop() {
    static_assert(
        ...
        "mpmc_queue requires a nothrow move constructor for T");
    std::size_t pos = dequeue_pos_.value.load(std::memory_order_relaxed);
    slot* cell = nullptr;
    for (;;) {
      cell = &slots_[pos % Capacity];
 ...
 private:
  ...
  counter enqueue_pos_;
  counter dequeue_pos_;
};
